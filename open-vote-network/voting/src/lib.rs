...
//! The protocol allows for decentralized privacy-preserving online voting,
//! as defined here: http://homepages.cs.ncl.ac.uk/feng.hao/files/OpenVote_IET.pdf
...
/// REGISTRATION PHASE: function voters call to register them for the vote by sending
/// (voting key, ZKP, deposit)
#[receive(
    contract = "voting",
    name = "register",
...
fn register<S: HasStateApi>(
    ...
    // Check correct amount is paid
    ensure!(
        host.state().config.deposit == deposit,
        types::RegisterError::WrongDeposit
    );

    // Check that registration is before registration end time
    ensure!(
...
/// COMMIT PHASE: function voters call to submit reconstructed key
/// and commit to their vote (by sending a hash of it)
#[receive(
    contract = "voting",
    name = "commit",
...
fn refund_deposits<S: HasStateApi>(
    ...
    // Reward sender (caller of change_phase) if they are not a stalling voter
    // and there were honest voters
